...
//     document.body.appendChild(simpsonElement);
//     }
// )
...
simpsons.forEach(element => {
    ...
    for (const simpsonKey in element) {
        if (simpsonKey !== 'photo') {
            const divChild = document.createElement('div');
            divChild.innerText = `${simpsonKey} - ${element[simpsonKey]}`;
...
coursesArray.forEach(item => {
    ...
    divWrap.style.border = '2px solid silver';
    ...
    monthDuration.style.flex = '1 0 0';
    monthDuration.innerText = `Тривалість - ${item.monthDuration} місяців`;
    const hourDuration = document.createElement('div');
    hourDuration.style.flex = '3 0 0';
    hourDuration.innerText = `Тривалість - ${item.hourDuration} годин`;
    ...
    if (item.modules) {
        for (const module of item.modules) {
            ...
            ul.appendChild(liModules);
            liModules.innerText = module;
        }
    }
    document.body.appendChild(divWrap);
    divWrap.append(title, divDuration, modules);
